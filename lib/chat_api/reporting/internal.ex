          ...
          "fields" => [
            ...
            %{
              ...
              *Most active accounts:*
              #{metrics.current.active_accounts_with_messages |> Enum.map(fn r -> "> #{r.account.company_name} (#{r.count} messages)" end) |> Enum.slice(0..4) |> Enum.join("\n")}
            ...
            %{
              ...
              "text" => """
              *Most active users:*
              #{metrics.current.active_users_with_messages |> Enum.map(fn r -> "> #{r.user.email} (#{r.count} messages)" end) |> Enum.slice(0..4) |> Enum.join("\n")}
            ...
            %{
              ...
              *New customers by account:*
              #{metrics.current.num_customers_by_account |> Enum.map(fn r -> "> #{r.account.company_name} (#{r.count} new customers)" end) |> Enum.slice(0..4) |> Enum.join("\n")}
